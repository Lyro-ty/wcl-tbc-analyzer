"""Backfill table data (ability breakdowns, buff uptimes) for existing reports."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Backfill ability/buff table data for existing reports",
    )
...
async def run(report_code: str) -> None:
    ...
    logger.info("Backfilled %d table data rows for report %s", total, report_code)
